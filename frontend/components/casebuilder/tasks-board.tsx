...
import { matterClaimsHref, matterDocumentHref, matterHref } from "@/lib/casebuilder/routes"
...
export function TasksBoard({ matter }: { matter: Matter }) {
  ...
  const dueSoon = openTasks.filter((task) => task.due_date && task.due_date <= dueSoonCutoff)
    ...
    <div className="flex flex-1 flex-col overflow-y-auto scrollbar-thin">
      ...
      <main className="px-6 py-6">
        {readiness.setupGaps.length > 0 && (
          <section className="mb-4 rounded border border-warning/30 bg-warning/10 p-4">
            <div className="flex items-center gap-2">
              <AlertTriangle className="h-4 w-4 text-warning" />
              <h2 className="text-sm font-semibold text-foreground">Recommended setup work</h2>
            </div>
            <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
              {readiness.setupGaps.map((gap) => (
                <Link
                  key={gap.title}
                  href={matterHref(matter.id, gap.href)}
                  className="rounded border border-warning/30 bg-card p-3 text-xs transition-colors hover:border-warning/60"
                >
