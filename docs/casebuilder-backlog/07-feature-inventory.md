...
| 2 | File / Evidence Ingestion | Partial | `CB-V0-002`, `CB-V0-003`, `CB-V0-011` |
...
| 12 | Legal Authority Finder | Partial | `CB-V0-012`, `CB-V02-008` |
| 13 | Drafting Studio | Partial | `CB-V0-014`, `CB-V0-015` |
| 14 | Complaint Builder | Partial | `CB-V0-013` |
...
|---|---|---|
| Upload all case files and automatically build a case graph | Partial | Backend storage and graph node foundation exist; upload UI, binary parsing, extraction, and automatic graph build remain. |
| Turn documents into reviewable facts with source citations | Partial | Deterministic proposed facts now include structured source spans/quotes; provenance review UI, duplicate handling, and richer extraction remain. |
| Build a chronological timeline automatically | Partial | Timeline page exists; live event creation and extraction from facts/documents remain. |
...
| Map facts/evidence to legal elements | Partial | Models and map endpoint exist; frontend persistence and richer element templates remain. |
| Find ORS authority for each claim/defense | Partial | Backend authority search exists; frontend attach/recommend UI and currentness panels remain. |
| Draft complaint/answer/motions from graph-backed facts | Partial | Draft scaffold exists for complaint-style drafts; answer and motion builders remain. |
| Fact-check every sentence in a draft | Partial | Paragraph-level deterministic checks exist; sentence nodes and source-backed live checks remain. |
| Show unsupported allegations before filing | Partial | Backend can create unsupported-fact findings; QC and editor need live wiring. |
| Build exhibit list and filing packet | Stub / Todo | Export routes return deferred errors; exhibit builder, packet generation, and download status remain. |
...
| Element mapping | Partial / deterministic | `CB-V0-010`, `CB-V0-024`, `CB-V02-009` |
| Authority retrieval | Partial / live ORSGraph search | `CB-V0-025`, `CB-V02-008`, `CB-X-006` |
| Drafting/editor | Partial / template | `CB-V0-014`, `CB-V0-015`, `CB-V0-026` |
...
| ORS authority search | Partial |
| Complaint builder | Partial |
